...
const VerStorageInv = ({ storageId: propStorageId, onClose }) => {
  ...
  const { storageId: urlStorageId } = useParams();
  const storageId = propStorageId || urlStorageId;
  const isModal = !!propStorageId;
  ...
  return (
    <div className={`${isModal ? "p-6" : "min-h-screen pb-12"} bg-[#fcfcfc] text-gray-800 dark:text-slate-100 font-sans print:bg-white dark:bg-slate-800 print:pb-0`}>
      ...
      {/* Header */}
      <header className={`w-full px-8 py-5 flex justify-between items-center border-b border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-800/80 backdrop-blur-md rounded-t-xl mb-4 print:hidden ${isModal ? "rounded-t-xl mb-4" : ""}`}>
        ...
        <div className="flex items-center gap-2">
          ...
          {!isModal && (
            <>
              ...
              <button
                onClick={() => navigate(`${BASE_PATH}/editar/${storageId}/storage-inv`)}
