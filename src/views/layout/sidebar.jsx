...
export default function Sidebar() {
  ...
  const Menus = [
    { id: 1, title: "Servidores Fisicos", icon: Server, path: "/inveplus/servidoresf" },
    { id: 2, title: "Servidores Virtuales", icon: Cloud, path: "/inveplus/servidoresv" },
    { id: 3, title: "Bases de Datos", icon: Database, path: "/inveplus/base-de-datos" },
    { id: 4, title: "Pseries", icon: Server, path: "/inveplus/pseries" },
    { id: 5, title: "Storage", icon: HardDrive, path: "/inveplus/storage" },
    { id: 6, title: "Sucursales", icon: Building, path: "/inveplus/sucursales" },
  ];
  ...
  return (
    <div className="flex h-screen">
      <div className={`flex flex-col justify-center  ${open ? "w-72" : "w-20"} bg-gray-800 h-full p-5 pt-10 relative duration-300 border-r border-gray-700`}>
        <img
          src="./src/assets/chevron-left.png"
          className={`absolute cursor-pointer -right-3 top-9 w-7 border-gray-700 bg-gray-800 border-2 rounded-full ${!open && "rotate-180"}`}
          onClick={() => setOpen(!open)}
        />
        <div className="gap-x-8 items-center mb-32">
          <img src="./src/assets/logowhite.png" className={`cursor-pointer duration-500 ${open}`} />
        </div>
        <ul className="pt-6">
          {Menus.map((Menu) => (
            <li
              ...
              className={`flex rounded-md p-2 cursor-pointer hover:bg-gray-700 text-gray-300 text-sm items-center gap-x-8 
                ${Menu.gap ? "mt-9" : "mt-2"} ${
                location.pathname.includes(Menu.path.split("/").pop()) && "bg-gray-700"
              }`}
